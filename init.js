...
(function initEmbers(){
  ...
  function frame(ts){
    if(!ts) ts = 0;
    // Reset si lag important (onglet en arrière-plan, économie d'énergie)
  ...
  requestAnimationFrame(frame);
  console.log('[Arya] Braises activées (persistantes)');
})();
...
(function applySeasonColor(){
  ...
  let c;
  if(m>=2&&m<=4) c='#4ade80';       // Printemps — Vert Tyrell
  else if(m>=5&&m<=7) c='#c44d6e';  // Été — Rouge Targaryen
  else if(m>=8&&m<=10) c='#d4a054'; // Automne — Or Lannister
  else c='#9aa5b4';                  // Hiver — Gris Stark
  const s = document.createElement('style');
  s.textContent=`.p-fill{background:${c}!important}.p-fill::after{background:${c}!important}.top-bar-fill{background:${c}!important}.fs-fill{background:${c}!important}.fs-fill::after{background:${c}!important}`;
...
function check316(t){
  ...
  if(s===189 && !_316shown){
    ...
    const purpleStyle = 'color:#9b30d4;font-size:9px;white-space:nowrap;font-weight:700;text-shadow:0 0 8px rgba(155,48,212,0.6);';
    el.textContent = '☠️ Noces Pourpres';
    el.style.cssText = purpleStyle;
    if(fsEl){ fsEl.textContent = '☠️ Noces Pourpres'; fsEl.style.cssText = purpleStyle; }
    _launchBloodDrips();
    setTimeout(()=>{
      el.textContent = originalText || fmtTime(t);
...
function _launchBloodDrips(){
  ...
  // Génère N traînées indépendantes
  const N = 18;
  const drips = [];
  for(let i = 0; i < N; i++){
    const x       = Math.random() * canvas.width;
    const w       = 4 + Math.random() * 10;          // épaisseur de la trainée
    const speed   = 60 + Math.random() * 160;         // px/s
    const delay   = Math.random() * 2000;             // ms avant départ
    // Points de contrôle pour un chemin légèrement sinueux
    const wobble  = (Math.random()-0.5) * 18;
    // Hue : rouge sang (0°) avec une légère variation
    const hue     = Math.floor(Math.random() * 10);
    const sat     = 80 + Math.random() * 20;
    drips.push({ x, w, speed, delay, wobble, hue, sat,
      y: 0,        // tip du filet en cours
      started: false,
      done: false,
      startTime: null,
    });
  ...
  const LIFETIME = 7000; // ms totales de l'animation

  function drawBloodTrail(d){
    if(!d.started || d.y <= 0) return;
    const h = d.y;
    // Trainée principale : rectangle légèrement effilé vers le bas
    const grad = ctx.createLinearGradient(d.x, 0, d.x, h);
    grad.addColorStop(0,   `hsla(${d.hue},${d.sat}%,18%,0.95)`); // haut : rouge très sombre
    grad.addColorStop(0.3, `hsla(${d.hue},${d.sat}%,28%,0.92)`);
    grad.addColorStop(0.7, `hsla(${d.hue},${d.sat}%,35%,0.85)`); // milieu : rouge vif
    grad.addColorStop(1,   `hsla(${d.hue},${d.sat}%,30%,0.6)`);  // bas : s'éclaircit un peu
    ctx.beginPath();
    // Forme effilée : large en haut, légèrement plus étroite en bas + wobble
    const topW  = d.w;
    const botW  = d.w * 0.55;
    const wx    = d.wobble * (h / canvas.height); // déviation latérale progressive
    ...
    // Droplet arrondie en bout de filet (effet "goutte qui tombe")
    const dropR = botW * 0.9 + 2;
    const cx    = d.x + wx;
    const cy    = h + dropR * 0.8;
    ...
    // Reflet brillant sur la goutte (highlight)
    ctx.beginPath();
  ...
  function frame(ts){
    ...
    for(const d of drips){
      ...
      const dt = (ts - d.startTime) / 1000;
      d.y = Math.min(d.speed * dt, canvas.height + 40);
    ...
    // Fade out global après 5.5s
    if(elapsed > 5500){
      const alpha = Math.max(0, 1 - (elapsed - 5500) / 600);
      ctx.globalAlpha = alpha;
      // On redessine tout avec alpha réduit via globalAlpha du canvas entier
      // (le canvas est déjà dessiné, on overlay un rect blanc transparent)
  ...
  raf = requestAnimationFrame(frame);
  setTimeout(()=>{ cancelAnimationFrame(raf); try{canvas.remove();}catch{}; }, LIFETIME + 200);
}
...
function logoClick(){
  ...
  if(_logoClicks>=3){
    ...
    if(vt && vt.textContent==='Toutes les chansons'){
      ...
      setTimeout(()=>{ vt.textContent=orig; document.title=origDocTitle; }, 5000);
    } else { setTimeout(()=>{ document.title=origDocTitle; }, 5000); }
  }
}
...
function showEyeClose(){
  ...
  if(!eye){
    eye = document.createElement('div'); eye.id = 'easterEye';
...
function _checkKonami(key){
  ...
  if(_konamiIdx===_KONAMI.length){
    _konamiIdx=0; if(_konamiActive) return; _konamiActive=true;
    const canvas=document.getElementById('emberCanvas');
    window._emberIntense=true; audio.playbackRate=0.82;
    toast('🔥 Valar Dohaeris');
    setTimeout(()=>{ window._emberIntense=false; audio.playbackRate=1; _konamiActive=false;
    }, 9000);
  }
}

// ── FRAPPE "valar" → SKIP (fonctionne partout, clavier physique) ─
let _keyBuf='';
window._keyBuf = _keyBuf; // expose sur window pour les nouveaux easter eggs
...
window._checkValar = _checkValar; // patchable
...
// Fallback pour clavier virtuel (mobile) : surveille la valeur de la recherche
(function(){
  let _lastSearch='';
...
// ── KONAMI (clavier physique, hors champ de texte) ─────────
document.addEventListener('keydown', e=>{
...
// ── KONAMI MOBILE (swipe: ↑↑↓↓←→←→ sur l'artwork du lecteur) ─────────
(function(){
  ...
  target.addEventListener('touchend', e=>{
    ...
    let dir=null;
    if(Math.abs(dx)<10&&Math.abs(dy)<10) return; // tap, ignorer
...
['click','keydown','mousemove','touchstart'].forEach(ev=>document.addEventListener(ev,_resetInact,{passive:true}));
_resetInact();

// ── 3 JOURS DE SUITE → COUTEAU ────────────
(function checkStreak(){
  const KEY='arya_visit_dates', today=new Date().toDateString();
  let dates=[];
...
// ── TRIPLE-TAP SUR POCHETTE (mobile) ─────
...
(function(){
  ...
  function _dracarysCheck(str){
    if(str.endsWith('dracarys') && !_dracarysActive){
  ...
  document.addEventListener('keydown', e=>{
    if(e.key.length!==1) return;
    _dracarysCheck((window._keyBuf||'').slice(-10));
  });
  const _si=document.getElementById('searchInput');
  if(_si) _si.addEventListener('input',()=>_dracarysCheck(_si.value.toLowerCase()));
})();
...
(function(){
  ...
  function _hodorCheck(str){
    if(str.endsWith('hodor') && !_hodorActive){
      _hodorActive=true;
      toast('HODOR.');
  ...
  document.addEventListener('keydown', e=>{
    if(e.key.length!==1) return;
    _hodorCheck((window._keyBuf||'').slice(-7));
  });
  const _si=document.getElementById('searchInput');
  if(_si) _si.addEventListener('input',()=>_hodorCheck(_si.value.toLowerCase()));
})();
...
(function(){
  ...
  function _winterCheck(str){
    if(str.endsWith('winter') && !_winterActive){
      _winterActive=true;
  ...
  const _si=document.getElementById('searchInput');
  if(_si) _si.addEventListener('input',()=>_winterCheck(_si.value.toLowerCase()));
})();
...
(function(){
  ...
  function _aryaCheck(str){
    if((str.endsWith('arya')||str==='arya') && !_aryaActive){
      _aryaActive=true;
      toast('🗡️ Un fille a une liste...');
  ...
  const _si=document.getElementById('searchInput');
  if(_si) _si.addEventListener('input',()=>_aryaCheck(_si.value.toLowerCase()));
})();
...
(function(){
  ...
  function _checkNightHour(){
    const now=new Date();
    if(now.getHours()===3 && now.getMinutes()===0 && !_nightShown){
      _nightShown=true;
      const el=document.createElement('div');
...
(function(){
  ...
  sfPseudo.addEventListener('click',()=>{
    ...
    if(_pseudoClicks>=7 && !_pseudoActive){
      ...
      // Passer accent en violet pendant 7s
