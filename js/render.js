...
function renderSongs() {
  ...
  list.innerHTML = songs.map((t, i) => {
    const sub = sortField === 'genre'
      ? (t.genre ? `${esc(t.genre)} · ${esc(t.artist)}` : esc(t.artist))
      : sortField === 'year'
        ? (t.year ? `${esc(t.artist)} · ${t.year}` : esc(t.artist))
        : esc(t.artist);
...
function showDetail(type, name) {
  ...
  const detailFiltered = ParentalControl.filterTracks(
    type === 'album'
  ...
  document.getElementById('detailList').innerHTML = detailFiltered.map((t, i) => {
    const faved = favs.has(t.filename);
    const sub   = esc(type === 'album' ? t.artist : t.album);
    return `
...
function onSearch() {
  ...
  filtered = q
    ? ParentalControl.filterTracks(tracks.filter(t => {
        const fields = [t.title, t.artist, t.album, t.genre, t.year].map(_normSearch);
        return tokens.every(tok => fields.some(f => f.includes(tok)));
      }))
    : ParentalControl.filterTracks([...tracks]);
