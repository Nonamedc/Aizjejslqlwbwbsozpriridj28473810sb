...
function _buildSuggestions() {
  ...
  const matchTrack = t => tokens.every(tok =>
    [t.title, t.artist, t.feat, t.album, t.genre, String(t.year || '')].some(f => _normSearch(f).includes(tok))
  );
    ...
    html += songSlice.map(t => {
      ...
      return `<div class="sug-item" role="option" onclick="_sugPlay(${t.id})">
        ...
        <div class="sug-info">
          ...
          <div class="sug-sub">${t.feat ? `${esc(t.artist)} feat. ${esc(t.feat)}` : esc(t.artist)} · ${esc(t.album)}</div>
        </div>
