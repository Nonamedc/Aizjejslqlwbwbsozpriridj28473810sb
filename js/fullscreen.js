...
function closeMoreSheet() {
  ...
  if (wasOpen && history.state?.arya === 'moreSheet') {
    window._aryaSheetClosing = true;
    history.back();
  }
}
...
// ── État initial History — empêche Android de quitter l'app ──
// On pousse deux entrées : replaceState (base) + pushState (buffer)
// Ainsi popstate a toujours quelque chose à consommer.
history.replaceState({ arya: 'base' }, '');
history.pushState(   { arya: 'app'  }, '');

window.addEventListener('popstate', e => {
  // Re-pousse immédiatement un état pour rester dans l'app
  history.pushState({ arya: 'app' }, '');

  // Si le popstate vient d'une fermeture de sheet → on ignore
  if (window._aryaSheetClosing) {
    window._aryaSheetClosing = false;
    return;
  }
  ...
  // Vue détail → retour (uniquement si on était pas sur dashboard)
  ...
  // Déjà sur l'accueil → ne rien faire (pushState empêche la fermeture)
  // NE PAS rediriger vers dashboard depuis les autres vues —
  // ça casserait la navigation normale depuis la sidebar
});
...
// Registre des renderers lazy — chaque vue peut enregistrer son renderer
window._lazyRenderers = window._lazyRenderers || {};
window.registerLazyRender = function(viewName, fn) {
  window._lazyRenderers[viewName] = fn;
};

// Hook sur showView — ajout des lazy renders SANS remplacer window.showView
...
function _onViewChanged(name) {
  // Rendus lazy built-in
  const _renders = {
    playlists:   () => typeof renderPlaylists   === 'function' && renderPlaylists(),
    leaderboard: () => typeof renderLeaderboard === 'function' && renderLeaderboard(),
    eq:          () => typeof renderEqPanel     === 'function' && renderEqPanel('eq-panel-container'),
    offline:     () => typeof renderOffline     === 'function' && renderOffline(),
    stats:       () => typeof renderStats       === 'function' && renderStats(),
    online:      () => typeof renderOnline      === 'function' && renderOnline(),
    party:       () => typeof renderParty       === 'function' && renderParty(),
    recent:      () => typeof initRecentView    === 'function' && initRecentView(),
    flow:        () => typeof Flow !== 'undefined' && Flow.render?.(),
  };
  try { _renders[name]?.(); } catch(e) { console.warn('[Arya] Lazy render:', name, e); }

  // Renderer enregistré dynamiquement
  try { window._lazyRenderers[name]?.(); } catch(e) {}

  // Badge nav mobile
  const mainViews = ['dashboard', 'songs', 'favorites', 'albums', 'artists', 'offline'];
  const mnMore    = document.getElementById('mnMoreBtn');
  if (mnMore) {
    mnMore.classList.toggle('active', !mainViews.includes(name) && MORE_VIEWS.includes(name));
  }
