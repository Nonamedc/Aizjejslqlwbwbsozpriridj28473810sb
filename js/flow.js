...
const Flow = (() => {
  ...
  function _buildScores() {
    ...
    /* ── Top artistes (score normalisé 0-1) — inclut les featurings ── */
    const artistPlays = {};
    Object.values(stats).forEach(e => {
      if (!e.artist) return;
      artistPlays[e.artist] = (artistPlays[e.artist] || 0) + (e.plays || 0);
    });
    // Crédite aussi les featurings dans les écoutes de la piste
    tracks.forEach(t => {
      if (!t.feat) return;
      const s = stats[t.filename];
      if (!s?.plays) return;
      (t.feat.split(/\s*,\s*|\s+(?:&|and|et|x)\s+/i)).forEach(name => {
        name = name.trim();
        if (name) artistPlays[name] = (artistPlays[name] || 0) + s.plays;
      });
    });
    ...
    return filterAllTracks(tracks).filter(t => !blocked.has(t.filename)).map(t => {
  ...
  function _patchPlayFromQueue() {
    ...
    patched._flowPatched = true;
    patched._orig        = orig;   // ← référence sauvegardée pour _unpatch
    window.playFromQueue = patched;
    _patching = true;
  }
            ...
            return `
              <div class="flow-preview-row" onclick="Flow.jumpTo(${_active ? queueIdx + i : -1}, '${escAttr(t.filename)}')">
                ...
                <div class="flow-preview-info">
                  ...
                  <div class="flow-preview-artist">${t.feat ? `${esc(t.artist)} feat. ${esc(t.feat)}` : esc(t.artist)}</div>
                </div>
  ...
  function start() {
    ...
    queue    = flowQueue;
    queueIdx = 0;
    // Désactive le shuffle UI sans écraser la variable globale de façon permanente
    if (shuffle) {
      shuffle = false;
  ...
  return { start, stop, shuffle: shuffle_, jumpTo, isActive, init, renderFlowView, generateQueue: _generateQueue };
