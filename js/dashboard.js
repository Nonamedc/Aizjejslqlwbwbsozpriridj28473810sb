...
async function _saveProfilePhoto(dataOrUrl) {
  localStorage.setItem('arya_profile_photo', dataOrUrl);
  // Sync Firebase (seulement si c'est une URL, pas base64 — trop lourd pour Firebase)
  if (typeof _uid !== 'undefined' && _uid && !dataOrUrl.startsWith('data:')) {
    firebase.database().ref(`users/${_uid}/photoUrl`).set(dataOrUrl).catch(() => {});
  }
  toast('✅ Photo de profil mise à jour');
  renderProfileView(); // rafraîchit
  if (typeof _updateSidebarUser === 'function') _updateSidebarUser();
}
