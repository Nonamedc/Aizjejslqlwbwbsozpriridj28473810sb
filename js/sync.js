  ...
  async function _loadFromFirebase() {
    try {
      ...
      if (Array.isArray(data.blocklist)) {
        localStorage.setItem(BLOCK_STORE, JSON.stringify(data.blocklist));
      }
      ...
      if (data.parental && typeof data.parental === 'object') {
        if (data.parental.enabled !== undefined)
          localStorage.setItem('arya_parental_enabled', data.parental.enabled);
        if (data.parental.artists)
