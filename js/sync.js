...
const Sync = (() => {
  ...
  async function _loadFromFirebase() {
    try {
      ...
      /* ── Stats mensuelles ── */
      if (data.statsMonthly && typeof data.statsMonthly === 'object') {
        const local  = JSON.parse(localStorage.getItem(STATS_MONTHLY_STORE) || '{}');
        const merged = { ...data.statsMonthly, ...local };
        localStorage.setItem(STATS_MONTHLY_STORE, JSON.stringify(merged));
      }

      /* ── Blocklist — fusion (local prioritaire) ── */
      if (Array.isArray(data.blocklist)) {
        const local  = new Set(JSON.parse(localStorage.getItem(BLOCK_STORE) || '[]'));
        const merged = [...new Set([...data.blocklist, ...local])];
        localStorage.setItem(BLOCK_STORE, JSON.stringify(merged));
      }
      ...
      /* ── Contrôle parental ── */
      if (data.parental && typeof data.parental === 'object') {
        if (data.parental.enabled !== undefined)
          localStorage.setItem('arya_parental_enabled', data.parental.enabled ? 'true' : 'false');
        if (Array.isArray(data.parental.artists))
          localStorage.setItem('arya_parental_allowed_artists', JSON.stringify(data.parental.artists));
      }

      _ready = true;
      if (typeof applyMeta === 'function') applyMeta();
      if (typeof BlockList === 'undefined' || true) {} // refresh après chargement
  ...
  function pushStatsMonthly() {
    if (!_uid) return;
    const m = JSON.parse(localStorage.getItem(STATS_MONTHLY_STORE) || '{}');
    _write('statsMonthly', m);
  }

  function pushBlocklist() {
    if (!_uid) return;
    const b = JSON.parse(localStorage.getItem(BLOCK_STORE) || '[]');
    _write('blocklist', b);
  }

  function pushParental() {
    if (!_uid) return;
    _write('parental', {
      enabled: localStorage.getItem('arya_parental_enabled') || 'false',
      artists: JSON.parse(localStorage.getItem('arya_parental_allowed_artists') || '[]'),
    });
  }
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    Object.keys(_writeQueue).forEach(path => {
      clearTimeout(_writeQueue[path]);
      delete _writeQueue[path];
    });
    // Flush immédiat vers Firebase (sans debounce)
    _writeNow('favs',         JSON.parse(localStorage.getItem(FAV_STORE)           || '[]'));
    _writeNow('history',      JSON.parse(localStorage.getItem(HIST_STORE)          || '[]').slice(0, 100));
    _writeNow('stats',        JSON.parse(localStorage.getItem(STATS_STORE)         || '{}'));
    _writeNow('statsMonthly', JSON.parse(localStorage.getItem(STATS_MONTHLY_STORE) || '{}'));
    _writeNow('meta',         JSON.parse(localStorage.getItem(META_STORE)          || '{}'));
    _writeNow('playlists',    JSON.parse(localStorage.getItem(PLAYLIST_STORE)      || '{}'));
    _writeNow('artistImgs',   JSON.parse(localStorage.getItem(ARTIST_IMG_STORE)    || '{}'));
    _writeNow('blocklist',    JSON.parse(localStorage.getItem(BLOCK_STORE)         || '[]'));
    _writeNow('lrcCache',     JSON.parse(localStorage.getItem(LRC_STORE)           || '{}'));
    _writeNow('parental', {
      enabled: localStorage.getItem('arya_parental_enabled') || 'false',
      artists: JSON.parse(localStorage.getItem('arya_parental_allowed_artists') || '[]'),
    });
