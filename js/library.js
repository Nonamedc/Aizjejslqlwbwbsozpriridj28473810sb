...
function renderFavorites() {
  ...
  const favs      = getFavs();
  const favTracks = ParentalControl.filterTracks(tracks.filter(t => favs.has(t.filename)));
...
function renderHistory() {
  ...
  const allowed = new Set(ParentalControl.filterTracks(tracks).map(t => t.filename));
      ...
      ${topTracks.map((t, i) => `
        <div class="top-track-row">
          ...
          <div class="top-bar-wrap">
            ...
            <div class="top-bar-sub">${esc(t.artist)}${t.seconds ? ` · ${fmtDuration(t.seconds)}` : ''}</div>
