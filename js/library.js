...
function toggleFavById(id, e) {
  ...
  const favs   = getFavs();
  const adding = !favs.has(t.filename);
  if (adding) favs.add(t.filename); else favs.delete(t.filename);
  saveFavs(favs);

  // Met à jour tous les boutons fav de cette piste dans le DOM
...
function renderFavorites() {
  ...
  const favs      = getFavs();
  const favTracks = filterAllTracks(tracks.filter(t => favs.has(t.filename)));

  if (count) count.textContent = `${favTracks.length} chanson${favTracks.length !== 1 ? 's' : ''}`;

  if (!favTracks.length) {
    list.innerHTML = `
      <div style="color:var(--text2);padding:60px;text-align:center;line-height:2;">
        ♡<br>Aucun favori pour l'instant<br>
        <small style="color:var(--text3);">Survolez une chanson et cliquez ♡ pour l'ajouter</small>
      </div>`;
    return;
  }
  ...
  list.innerHTML = favTracks.map((t, i) => `
    <div class="track-row${currentId === t.id ? ' playing' : ''}" data-id="${t.id}" onclick="playTrack(${t.id})">
      ...
      <div class="tr-artist">${artistFeatHtml(t)}</div>
      <div class="tr-album">${esc(t.album)}</div>
      <div class="tr-dur">
        <span class="dur-val">${fmtTime(t.length)}</span>
        <div class="tr-actions">
          <button class="ico-btn fav-btn active" data-tid="${t.id}"
                  onclick="toggleFavById(${t.id}, event)" title="Retirer des favoris">♥</button>
        </div>
      </div>
    </div>`).join('');
}
...
function addToHistory(t) {
  const h = getHistory();
  // Déduplique : même piste dans les 5 min (crossfade ou retry)
  if (h.length && h[0].filename === t.filename && (Date.now() - h[0].ts) < 300_000) return;
...
function renderHistory() {
  ...
  const allowed = new Set(filterAllTracks(tracks).map(t => t.filename));
  const h = getHistory().filter(e => !ParentalControl.isEnabled() || allowed.has(e.filename));

  if (count) count.textContent = `${h.length} écoute${h.length !== 1 ? 's' : ''}`;
  ...
  list.innerHTML = h.map((e, i) => {
    ...
    return `
      <div class="track-row hist-track-row${currentId === id ? ' playing' : ''}" data-id="${id}" ${id >= 0 ? `onclick="playTrack(${id})"` : ''}>
        ...
        <div class="tr-info">
          ...
          <div class="tr-artist-s">${t ? artistFeatHtml(t) : esc(e.artist)}</div>
        </div>
        <div class="tr-artist">${t ? artistFeatHtml(t) : esc(e.artist)}</div>
...
function getStats() {
  try { return JSON.parse(localStorage.getItem(STATS_STORE) || '{}'); }
  catch { return {}; }
}

function saveStats(s) {
  localStorage.setItem(STATS_STORE, JSON.stringify(s));
}
...
/* ═══════════════════════════════════════════════════════════
   STATS SUMMARY — helper partagé par renderStats, renderProfileView, renderDashboard
═══════════════════════════════════════════════════════════ */

/**
 * Calcule un résumé complet des statistiques d'écoute.
 * Évite de recalculer la même chose dans dashboard, profil et stats.
 * @returns {{ totalPlays, totalSec, topTracks, topArtists, artistMap }}
 */
function getStatsSummary() {
  ...
  entries.forEach(e => {
    ...
    artistMap[e.artist].plays   += e.plays   || 0;
    artistMap[e.artist].seconds += e.seconds || 0;
  });
  const topArtists = Object.entries(artistMap).sort((a, b) => b[1].plays - a[1].plays);

  return { totalPlays, totalSec, topTracks, topArtists, artistMap };
}


function renderStats() {
  const { totalPlays, totalSec, topTracks, topArtists } = getStatsSummary();

  const subtitle = document.getElementById('statsSubtitle');
  if (subtitle) {
    subtitle.textContent = totalPlays > 0
      ? `${totalPlays} lecture${totalPlays > 1 ? 's' : ''} · ${fmtDuration(totalSec)} au total`
      : '—';
  }
  ...
  content.innerHTML = `
      ...
      ${topTracks.map((t, i) => `
        <div class="top-track-row">
          ...
          <div class="top-bar-wrap">
            ...
            <div class="top-bar-sub">${t.feat ? esc(t.artist) + ' feat. ' + esc(t.feat) : esc(t.artist)}${t.seconds ? ` · ${fmtDuration(t.seconds)}` : ''}</div>
    ...
    <div>
      ...
      ${topArtists.slice(0, 6).map(([name, data]) => `
