...
async function getOfflineTracks() {
  ...
  /* ── 1. Via CacheManager (si cache.js est chargé) ───── */
  if (typeof CacheManager !== 'undefined' && CacheManager.isEnabled()) {
    try {
      // Tente l'API listCached si elle existe
      if (typeof CacheManager.listCached === 'function') {
        const cachedFilenames = await CacheManager.listCached();
        return tracks.filter(t => cachedFilenames.includes(
          (t.deezerUrl || t.url).split('/').pop().split('?')[0]
        ));
      }
    } catch (e) {
      console.warn('[Arya Offline] CacheManager.listCached failed, fallback to Cache API', e);
    }
  }

  /* ── 2. Cache Storage natif ─────────────────────────── */
  ...
  try {
    ...
    // Collecte toutes les URLs cachées
    ...
    // Croise avec les pistes connues
    return tracks.filter(t => {
      const url = t.deezerUrl || t.url;
      if (!url) return false;
      // Match exact ou partiel sur le nom de fichier
    ...
    console.warn('[Arya Offline] Cache API error:', e);
    return [];
  }
}
