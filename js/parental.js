/* ═══════════════════════════════════════════════════════════
   PARENTAL.JS — Contrôle Parental · Arya
...
const ParentalControl = (() => {
  ...
  function filterTracks(tracks) {
    ...
    return tracks.filter(t => {
      // Artiste principal autorisé ?
      if (allowed.some(a => (t.artist || '').toLowerCase().trim() === a)) return true;
      // Au moins un artiste en featuring autorisé ?
      if (t.feat) {
        const featNames = t.feat.split(/\s*,\s*|\s+(?:&|and|et|x)\s+/i).map(s => s.toLowerCase().trim());
        if (featNames.some(name => allowed.includes(name))) return true;
      }
      return false;
    });
  }
  ...
  async function _showSetupModal(allArtists = []) {
    ...
    overlay.querySelector('#pc-save').onclick = async () => {
      ...
      if (typeof Sync !== 'undefined') {
        firebase.database().ref(`users/${typeof _uid !== 'undefined' ? _uid : 'anon'}/parental`).set({
          enabled: active,
          artists: selected,
        }).catch(() => {});
      }
