...
function buildQueue() {
  if (shuffle) {
    // Utilise l'algorithme pondéré de Flow si des stats existent, sinon aléatoire pur
    const hasStats = typeof getStats === 'function' && Object.keys(getStats()).length > 0;
    const canFlow  = typeof Flow !== 'undefined' && hasStats;
    queue = canFlow ? Flow.generateQueue(tracks.length) : shuffleArr(filterAllTracks([...tracks]));
  } else {
    queue = filterAllTracks([...tracks]);
  }
...
function updatePlayerUI(t) {
  const artistLabel = t.feat ? `${t.artist} feat. ${t.feat}` : t.artist;
  document.getElementById('pTitle').textContent  = t.title;
  document.getElementById('pArtist').textContent = artistLabel + (t.album && t.album !== 'Sans album' ? ' · ' + t.album : '');
...
window.updatePlayerUI = function (t) {
  ...
  navigator.mediaSession.metadata = new MediaMetadata({
    title:  t.title  || '',
    artist: t.feat ? `${t.artist} feat. ${t.feat}` : (t.artist || ''),
    album:  t.album  || '',
    artwork,
  });
