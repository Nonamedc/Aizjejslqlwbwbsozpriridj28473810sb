...
(function _injectBulkStyles() {
  ...
  s.textContent = `
    ...
    .sel-mode .track-row.sel-checked::before {
    ...
    .sel-mode .track-row.sel-checked {
      background: color-mix(in srgb, var(--accent) 10%, transparent);
    }
    .sel-mode .track-row * { pointer-events: none; }
...
let _selMode      = false;
let _selContainer = null; // conteneur actif (trackList, favList, histList, etc.)
...
function _wrapRenderSongs() {
  // Liste de toutes les fonctions de rendu à wrapper
  const toWrap = ['renderSongs', 'renderFavorites', 'renderHistory', 'renderStats',
                  'renderAlbums', 'renderArtists', 'renderPlaylists'];
  toWrap.forEach(name => {
    const orig = window[name];
    if (!orig || orig._bulkWrapped) return;
    window[name] = function () {
      orig.apply(this, arguments);
      if (_selMode && _selContainer) _bulkReapplyUI();
    };
    window[name]._bulkWrapped = true;
  });
}
...
function _initBulkListeners() {
  if (document._bulkInited) return;
  document._bulkInited = true;

  /* Long press → entre en mode sélection */
  document.addEventListener('pointerdown', e => {
    const row = e.target.closest('.track-row');
    if (!row) return;
    // Ignore les rows dans le fullscreen player ou la queue
    if (e.target.closest('#fsQueuePanel, #fsPlayer, #bulkEditPanel')) return;
    _lpMoved  = false;
    _lpStartX = e.clientX;
    _lpStartY = e.clientY;
    ...
    _lpTimer = setTimeout(() => {
      if (_lpMoved) return;
      // Trouve le conteneur scrollable le plus proche
      const container = row.closest('[id]') || row.parentElement;
      if (!_selMode) _enterSelMode(container);
      _toggleSelect(id);
  ...
  document.addEventListener('pointerup',     _cancelLp, { passive: true });
  document.addEventListener('pointercancel', _cancelLp, { passive: true });

  /* Bloque le menu contextuel natif Android pendant le long press */
  document.addEventListener('contextmenu', e => {
    if (_selMode) e.preventDefault();
  }, { passive: false });
...
function _enterSelMode(container) {
  _selMode      = true;
  _selContainer = container || document.getElementById('trackList');
  _selected.clear();
...
function _exitSelMode() {
  ...
  _selected.clear();
  if (_selContainer) {
    _selContainer.classList.remove('sel-mode');
    _selContainer.querySelectorAll('.track-row.sel-checked')
...
function _toggleSelect(id) {
  ...
  const container = _selContainer || document;
  container.querySelector(`.track-row[data-id="${id}"]`)
    ?.classList.toggle('sel-checked', _selected.has(id));
  _updateSelBar();
}

function _bulkSelectAll() {
  // Sélectionne toutes les rows visibles dans le conteneur actif
  const container = _selContainer || document.getElementById('trackList');
  container.querySelectorAll('.track-row[data-id]').forEach(row => {
    const id = parseInt(row.dataset.id);
    if (!isNaN(id)) _selected.add(id);
  });
  _bulkReapplyUI();
  _updateSelBar();
}
...
function _bulkReapplyUI() {
  const container = _selContainer || document.getElementById('trackList');
  if (!container) return;
  container.classList.add('sel-mode');
  container.querySelectorAll('.track-row[data-id]').forEach(row => {
