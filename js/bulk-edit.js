/* ═══════════════════════════════════════════════════════
   BULK-EDIT.JS — Multi-sélection & édition en masse
   Arya Music Player

   Dépendances (chargé APRÈS) :
     config.js, render.js, covers.js, utils.js

   Point d'entrée : appeler initBulkEdit() depuis init.js
   après le premier renderAll().
═══════════════════════════════════════════════════════ */

/* ────────────────────────────────────────────────────
   0. CSS INJECTION (autonome, pas de fichier séparé)
──────────────────────────────────────────────────── */
...
/* Empêcher le click de déclencher le play sur fav-btn en sel-mode */
#trackList.sel-mode .track-row * {
  pointer-events: none;
}
...
#bulkEditPanel.visible .bep-sheet {
...
/* Champ image : préview + url/fichier */
.bep-img-row {
  display: flex;
  gap: 12px;
...
const LP_MOVE_THRESHOLD = 8; // pixels — seuil avant d'annuler le long press
...
function _initBulkListeners() {
  ...
  /* ── Long press : entrée en mode sélection ── */
  ...
  list.addEventListener('pointerup',     () => { clearTimeout(_lpTimer); _lpTimer = null; }, { passive: true });
  list.addEventListener('pointercancel', () => { clearTimeout(_lpTimer); _lpTimer = null; }, { passive: true });
  ...
  list.addEventListener('click', e => {
    if (!_selMode) return;
    // Bloque tout ce qui est dans un .track-row
    const row = e.target.closest('.track-row');
    if (!row) return;
    e.preventDefault();
    e.stopPropagation();
...
function _exitSelMode() {
  ...
  // Retire toutes les classes de sélection
...
function _toggleSelect(id) {
  ...
  const row = document.querySelector(`#trackList .track-row[data-id="${id}"]`);
  if (row) row.classList.toggle('sel-checked', _selected.has(id));
  _updateSelBar();
}
...
function _bulkReapplyUI() {
  ...
  list.querySelectorAll('.track-row').forEach(row => {
    const id = parseInt(row.dataset.id);
    row.classList.toggle('sel-checked', _selected.has(id));
  });
}
  ...
  if (!bar) {
    ...
    bar.innerHTML = `
      ...
      <div class="bsb-actions">
        ...
        <button class="bsb-btn close" onclick="_exitSelMode()" title="Quitter la sélection">✕</button>
      </div>
    `;
    document.body.appendChild(bar);
  }
  // Force reflow avant d'ajouter .visible pour que la transition joue
...
function openBulkEditPanel() {
  ...
  /* Détecte les valeurs mixtes entre les pistes sélectionnées */
  ...
  /* Valeur pré-remplie : uniquement si tous identiques */
  const single = arr => (arr.length === 1 ? arr[0] : '');
  /* Placeholder informatif si valeurs mixtes */
  ...
  albumInput.placeholder = phMixed(uAlbum, 'One Piece OST');
  ...
  genreInput.placeholder = phMixed(uGenre, 'Anime, J-Pop…');

  /* Cover : pré-remplie seulement si toutes identiques */
  const firstCover = single(uCovers);
  document.getElementById('bepCoverUrl').value = firstCover;
  _setBepCoverPreview(firstCover);

  /* Affiche la note si valeurs mixtes */
...
function _onBepCoverUrlInput(val) {
  _setBepCoverPreview(val.trim());
}
...
function saveBulkEdit() {
  ...
  /* Lit le store méta courant une seule fois (perf) */
  const metaStore = getMeta();

  selTracks.forEach(t => {
    /* Merge : ne modifie que les champs fournis, conserve les autres */
    const existing = metaStore[t.filename] || {};
    ...
    /* Rafraîchit les pochettes dans le DOM à la volée */
...
/* ────────────────────────────────────────────────────
   9. INIT — à appeler depuis init.js après renderAll()
──────────────────────────────────────────────────── */
