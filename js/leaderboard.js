/* ══════════════════════════════════════════════
   LEADERBOARD v3 — Firebase Realtime DB
...
function getMonthlyStats() {
  try { return JSON.parse(localStorage.getItem(STATS_MONTHLY_STORE) || '{}'); } catch { return {}; }
}
...
/* ─────────────────────────────────────────────
   PUSH VERS FIREBASE
   Appelé depuis playback.js après chaque écoute
───────────────────────────────────────────── */
function pushLeaderboardStats() {
  ...
  if (t) {
    ...
    const fn = t.filename;
    if (!monthly[monthKey][fn]) {
      monthly[monthKey][fn] = { plays: 0, artist: t.artist || '', title: t.title || '' };
    }
    monthly[monthKey][fn].plays++;
    localStorage.setItem(STATS_MONTHLY_STORE, JSON.stringify(monthly));
  }
  ...
  Object.entries(monthly).forEach(([monthKey, trackMap]) => {
  ...
  firebase.database().ref('leaderboard/' + pseudo).set({
    ...
    monthly:  monthlyData,
    yearly:   yearlyData,
  }).catch(e => console.warn('[Leaderboard] push error', e));
}
...
let _lbCachedData = null; // données Firebase en cache pour la vue profil

/* ─────────────────────────────────────────────
   RENDER PRINCIPAL (vue classement global)
───────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────
   VUE GLOBALE — classement tous utilisateurs
───────────────────────────────────────────── */
function _renderLbGlobal(data, content) {
  const users  = Object.values(data).filter(u => u && u.pseudo && (u.totalPlays || 0) > 0);
  ...
  // Classement complet — clic sur une ligne → vue profil
  ...
  sorted.forEach((u, i) => {
    ...
    html += `
    <div class="lb-row${isMe ? ' lb-row-me' : ''}" onclick="_renderLbProfile('${esc(u.pseudo)}')" style="cursor:pointer;">
  ...
  if (gTop10Artists.length) {
    ...
    gTop10Artists.forEach((a, i) => {
      ...
      <div class="lb-row">
        <div class="lb-rank">${medals[i] || i + 1}</div>
        <div style="width:36px;height:36px;border-radius:50%;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:14px;${img ? '' : 'background:' + grad}">
  ...
  if (gTop10Tracks.length) {
    ...
    gTop10Tracks.forEach((t, i) => {
      ...
      <div class="lb-row" ${tr ? `onclick="playTrack(${tr.id})" style="cursor:pointer;"` : ''}>
        <div class="lb-rank">${medals[i] || i + 1}</div>
        <div style="width:36px;height:36px;border-radius:7px;overflow:hidden;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:14px;${cover ? '' : 'background:' + grad}">
  ...
  if (yTop10Artists.length || yTop10Tracks.length) {
    html += `
    <div style="margin:32px 0 12px;padding:12px 16px;background:color-mix(in srgb,var(--accent) 10%,transparent);border:1px solid color-mix(in srgb,var(--accent) 40%,transparent);border-radius:14px;">
...
/* ─────────────────────────────────────────────
   VUE PROFIL — stats détaillées d'un utilisateur
   Accessible par tout le monde (clic sur pseudo)
───────────────────────────────────────────── */
function _renderLbProfile(targetPseudo) {
  const content = document.getElementById('leaderboardContent');
  if (!content) return;

  // Chercher l'utilisateur dans le cache Firebase
  const u = _lbCachedData
    ? Object.values(_lbCachedData).find(x => x && x.pseudo === targetPseudo)
    : null;
  ...
  <div style="
    ...
    <div style="
      ...
      font-size:26px;font-weight:700;color:var(--text1,#fff);
      flex-shrink:0;
  ...
  if (yData && (yData.totalPlays || 0) > 0) {
    html += `
    <div style="margin:28px 0 12px;padding:12px 16px;background:color-mix(in srgb,var(--accent) 10%,transparent);border:1px solid color-mix(in srgb,var(--accent) 40%,transparent);border-radius:14px;">
      <div style="font-size:16px;font-weight:700;color:var(--accent);">🏆 Récap ${yearKey}</div>
  ...
  if (allMonths.length > 1) {
    html += `
    <div class="lb-section-label" style="margin-top:28px;">📆 Historique mensuel</div>
  ...
  html += `
  <div style="margin-top:24px;margin-bottom:8px;font-size:11.5px;color:var(--text3);text-align:center;line-height:1.9;">
...
function _lbGoBack() {
  ...
  // On re-render depuis le cache (pas de re-fetch Firebase)
  if (_lbCachedData) {
    _renderLbGlobal(_lbCachedData, content);
  } else {
    renderLeaderboard();
