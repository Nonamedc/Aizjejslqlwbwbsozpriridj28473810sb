...
// STATS_MONTHLY_STORE défini dans config.js
...
/** Helper HTML avatar utilisateur (photo ou initiale). */
function _lbAvatarHtml(u, size = 44) {
  const initial = (u.pseudo || '?')[0].toUpperCase();
  const grad    = gradFor(u.pseudo || '');
  if (u.photoUrl) {
    return `<div style="width:${size}px;height:${size}px;border-radius:50%;overflow:hidden;
                flex-shrink:0;border:2px solid var(--border);">
      <img src="${esc(u.photoUrl)}" alt="${esc(u.pseudo)}"
           style="width:100%;height:100%;object-fit:cover;"
           onerror="this.parentElement.style.background='${grad}';
                    this.parentElement.innerHTML='<span style=\\'font-size:${Math.round(size*0.4)}px;color:#fff;font-weight:700;\\'>${initial}</span>';">
    </div>`;
  }
  return `<div style="width:${size}px;height:${size}px;border-radius:50%;
              background:${grad};flex-shrink:0;
...
function pushLeaderboardStats() {
  ...
  if (t) {
    ...
    if (typeof Sync !== 'undefined') Sync.pushStatsMonthly();
  }
  ...
  const photoUrl = localStorage.getItem('arya_profile_photo') || null;
...
function _renderLbGlobal(data, content) {
  ...
  sorted.slice(0, 3).forEach((u, i) => {
    ...
    <div class="lb-podium-col" onclick="_renderLbProfile('${esc(u.pseudo)}')" style="cursor:pointer;">
      <div class="lb-podium-avatar${isMe ? ' me' : ''}" style="height:${sz}px;width:${sz}px;overflow:hidden;${u.photoUrl ? 'padding:0;background:none;' : ''}">
        ${u.photoUrl
          ? `<img src="${esc(u.photoUrl)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;"
                  onerror="this.style.display='none';this.parentElement.textContent='${(u.pseudo||'?')[0].toUpperCase()}';">`
...
function _renderLbProfile(targetPseudo) {
  ...
  <div style="display:flex;align-items:center;gap:16px;padding:16px;
    ...
    <div style="${isMe ? 'outline:2px solid var(--accent);outline-offset:2px;border-radius:50%;' : ''}">
      ${_lbAvatarHtml(u, 60)}
    </div>
    <div>
